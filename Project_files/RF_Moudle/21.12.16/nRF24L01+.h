...
static unsigned char Send_To_nRF[Payload_Size]={0};
static unsigned char Receive_From_nRF[Payload_Size]={0};
	/*
int main(void)
{
...
void processRXorTX()
{
	...
	if (OneTime)
	{
		Mode = SetControllerRXorTX(RXorTX);
		OneTimeSetup();
		OneTime = 0;
	}
...
/*
	if (RXorTX)
	{
		...
		Send_int_decimal(count1);
		printString("Fail:");
		Send_int_decimal(count2);
		count1 = 0;
		count2 = 0;
	}
	else
	{
		printString("Received Value By Listen:");
		Send_int_decimal(count1);
		
		printString("Received Value From FIFO:");
		Send_int_decimal(count2);
		count1 = 0;
		count2 = 0;
	}
