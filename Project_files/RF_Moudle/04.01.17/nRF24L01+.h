...
extern unsigned char Send_To_nRF[Payload_Size];
extern unsigned char Receive_From_nRF[Payload_Size];
...
void CheckForDummyDATA()
{
		if (Receive_From_nRF[0] == Receive_From_nRF[1] && Receive_From_nRF[2] == Receive_From_nRF[3] && Receive_From_nRF[3] == Receive_From_nRF[4])
		{
			for (char i = 0; i < Payload_Size; i++)
			{
				//Receive_From_nRF[i] = 0x00;
			}
		}
...
void Transmite_Payload(unsigned char *Data_Transmission)
{
	...
	Check_Status[0] = Write_Data_To_nRF(STATUS, Check_Status,1);
	
	//check if the transmitter reached to max of retransmissions
	if((Check_Status[0] & (1 << MAX_RT)))
	{
	...
	//_delay_us(900);
	char received = 1,counterOUT = 0;
	int counterIN = 1;
	while(received)
	{
		
		received = ReadStatusRegister();
		USART_send(received);
		if (received & (1<<MAX_RT) || (received & (1<<TX_DS)))
		{
			received = 0;
			Reset_IRQ();
		}
...
char ReadStatusRegister()
{
	...
	STATUSRegister = SPI_Byte_TX_OR_RX(NOP);
	//end of command
	Send_Signal |= (1 << CSN);
...
{
	//this arr is for getting STATUS register value
	char test[1] = {1},check = 0;
	Send_Signal |= (1<<CE);
	while(test[0])
	{
		
		test[0] = check = ReadStatusRegister();
		check = test[0] & (1<<RX_DR);
	...
	if(((test[0] & (1 << 3))&&( test[0] & (0 << 2)))|| test[0] & (1 << RX_DR) )
	{
...
char RX_Process()
{
	...
	{
		//get the data that has been received into the receive arr
		if(Receive_Payload(Receive_From_nRF))
		{
			
			//count for receive packet
			count1++;
			//Check if the data we received is Dummy DATA
			//CheckForDummyDATA();
		}
		else
		{
...
void PrintSTSTUSRegister()
{
	...
	if(STATUSRegister == 0)
	{
		printString("STATUS Register = 0 ");
	}
	else 
	{
		if (STATUSRegister & (1 << RX_DR))
		{
			printString("RX_DR = 1 ,");
		} 
		else
		...
		if (STATUSRegister & (1 << MAX_RT))
		{
			printString("MAX_RT = 1 ,");
		}
		else
		{
			printString("MAX_RT = 0 ,");
		}
		if (STATUSRegister & (1 << TX_FULL))
		{
			printString("TX_FULL = 1 ,");
		}
		else
		{
			printString("TX_FULL = 0 ,");
		}
		if (STATUSRegister & (1 << 1)&&STATUSRegister & (1 << 2)&&STATUSRegister & (1 << 3))
		{
			printString("RX FIFO Empty ,");
		}
		else
		{
			printString("Available Data On Pipe Number: ");
			USART_send((STATUSRegister= STATUSRegister & 0x0E) +'0');
			printString(" ,");
		}
	}
}